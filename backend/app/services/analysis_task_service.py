...
from app.models.error_model import get_error
...
class AnalysisTaskService:
    ...
    def analyze_zip_project(cls, project_id: str, zip_path: Path, project_root: Path) -> None:
        ...
        try:
            ProjectService.update_project_status(project_id, "running", "scanning", 20, "正在解压 zip 项目")
            cls._safe_extract_zip(zip_path, project_root)

            ProjectService.update_project_status(project_id, "running", "scanning", 55, "正在扫描项目目录")
            files = ScanService.scan_project(project_root)
            if not files:
                error = get_error("NO_SUPPORTED_FILES")
                cls._mark_failed(project_id, error.code, error.message, error.suggestion, ValueError(error.message))
                return

            ProjectService.update_project_status(project_id, "running", "analyzing", 80, "正在抽取工程规范")
            ...
            error = get_error("ZIP_INVALID")
            cls._mark_failed(project_id, error.code, error.message, error.suggestion, exc)
        except ValueError as exc:
            error = get_error("SCAN_FAILED", str(exc))
            cls._mark_failed(project_id, error.code, error.message, error.suggestion, exc)
        except Exception as exc:
            error = get_error("ANALYZE_FAILED", "上传项目分析失败", "请检查 zip 内容或稍后重试。")
            cls._mark_failed(project_id, error.code, error.message, error.suggestion, exc)
    ...
    def analyze_repo_project(cls, project_id: str, git_url: str, repo_dir: Path, access_token: str | None = None) -> None:
        """后台克隆仓库并执行扫描、分析、结果落盘。"""
        try:
            ProjectService.update_project_status(project_id, "running", "cloning", 25, "正在克隆远程仓库")
            try:
                RepoService.clone_repo(git_url, repo_dir, access_token)
            except TimeoutError as exc:
                error = get_error("REPO_CLONE_FAILED", "仓库克隆超时", "请确认仓库体积和网络状态后重试。")
                cls._mark_failed(project_id, error.code, error.message, error.suggestion, exc)
                return
            except ValueError as exc:
            ...
            ProjectService.update_project_status(project_id, "running", "scanning", 60, "正在扫描项目目录")
            files = ScanService.scan_project(repo_dir)
            if not files:
                cls._mark_failed(
                    project_id,
                    "NO_SUPPORTED_FILES",
                    get_error("NO_SUPPORTED_FILES").message,
                    get_error("NO_SUPPORTED_FILES").suggestion,
                    ValueError("no analyzable source files"),
                )
                return

            ProjectService.update_project_status(project_id, "running", "analyzing", 85, "正在抽取工程规范")
            analysis = PatternAnalyzerService.analyze(files)
            ...
            error = get_error("SCAN_FAILED", str(exc), "请检查仓库源码结构后重试。")
            cls._mark_failed(project_id, error.code, error.message, error.suggestion, exc)
        except Exception as exc:
            error = get_error("ANALYZE_FAILED", "仓库分析失败", "请确认仓库存在、权限可访问且网络正常。")
            cls._mark_failed(project_id, error.code, error.message, error.suggestion, exc)
